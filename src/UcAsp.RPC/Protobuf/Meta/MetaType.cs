...
using System.Text;
using UcAsp.RPC.ProtoBuf.Serializers;
...
{
    /// <summary>
    /// Represents a type at runtime for use with UcAsp.RPC.ProtoBuf, allowing the field mappings (etc) to be defined
    /// </summary>
    public class MetaType : ISerializerProxy
    {
        ...
        private IProtoTypeSerializer BuildSerializer()
        {
            if (Helpers.IsEnum(type))
            {
                return new TagDecorator(UcAsp.RPC.ProtoBuf.Serializer.ListItemTag, WireType.Variant, false, new EnumSerializer(type, GetEnumMap()));
            }
            Type itemType = IgnoreListHandling ? null : TypeModel.GetListItemType(model, type);
            if (itemType != null)
            {
                ...
                ValueMember fakeMember = new ValueMember(model, UcAsp.RPC.ProtoBuf.Serializer.ListItemTag, type, itemType, defaultType, DataFormat.Default);
                return new TypeSerializer(model, type, new int[] { UcAsp.RPC.ProtoBuf.Serializer.ListItemTag }, new IProtoSerializer[] { fakeMember.Serializer }, null, true, true, null, constructType, factory);
            }
            if (surrogate != null)
            {
        ...
        internal static bool GetAsReferenceDefault(RuntimeTypeModel model, Type type)
        {
            ...
            {
                if (typeAttribs[i].AttributeType.FullName == "UcAsp.RPC.ProtoBuf.ProtoContractAttribute")
                {
                    object tmp;
        ...
        internal void ApplyDefaultBehaviour()
        {
            ...
            for (int i = 0; i < typeAttribs.Length; i++)
            {
                ...
                if (!isEnum && fullAttributeTypeName == "UcAsp.RPC.ProtoBuf.ProtoIncludeAttribute")
                {
                    int tag = 0;
                ...
                if (fullAttributeTypeName == "UcAsp.RPC.ProtoBuf.ProtoPartialIgnoreAttribute")
                {
                ...
                if (!isEnum && fullAttributeTypeName == "UcAsp.RPC.ProtoBuf.ProtoPartialMemberAttribute")
                {
                    if (partialMembers == null) partialMembers = new BasicList();
                    partialMembers.Add(item);
                }

                if (fullAttributeTypeName == "UcAsp.RPC.ProtoBuf.ProtoContractAttribute")
                {
            ...
            foreach (MemberInfo member in foundList)
            {
                ...
                else if ((method = member as MethodInfo) != null)
                {
                    ...
                    {
                        CheckForCallback(method, memberAttribs, "UcAsp.RPC.ProtoBuf.ProtoBeforeSerializationAttribute", ref callbacks, 0);
                        CheckForCallback(method, memberAttribs, "UcAsp.RPC.ProtoBuf.ProtoAfterSerializationAttribute", ref callbacks, 1);
                        CheckForCallback(method, memberAttribs, "UcAsp.RPC.ProtoBuf.ProtoBeforeDeserializationAttribute", ref callbacks, 2);
                        CheckForCallback(method, memberAttribs, "UcAsp.RPC.ProtoBuf.ProtoAfterDeserializationAttribute", ref callbacks, 3);
        ...
        internal static AttributeFamily GetContractFamily(RuntimeTypeModel model, Type type, AttributeMap[] attributes)
        {
            ...
            for (int i = 0; i < attributes.Length; i++)
            {
                ...
                {
                    case "UcAsp.RPC.ProtoBuf.ProtoContractAttribute":
                        bool tmp = false;
        ...
        internal static ConstructorInfo ResolveTupleConstructor(Type type, out MemberInfo[] mappedMembers)
        {
            ...
            // for most types we'll enforce that you need readonly, because that is what UcAsp.RPC.ProtoBuf-net
            ...
            for (int i = 0; i < fieldsPropsUnfiltered.Length; i++)
            {
                ...
                {
                    prop = (PropertyInfo)fieldsPropsUnfiltered[i];
                ...
                else
                {
                    ...
                    {
                        field = (FieldInfo)fieldsPropsUnfiltered[i];
        ...
        private static ProtoMemberAttribute NormalizeProtoMember(TypeModel model, MemberInfo member, AttributeFamily family, bool forced, bool isEnum, BasicList partialMembers, int dataMemberOffset, bool inferByTagName, ref bool hasConflictingEnumValue, MemberInfo backingMember = null)
        {
            ...
            if (isEnum)
            {
                attrib = GetAttribute(attribs, "UcAsp.RPC.ProtoBuf.ProtoIgnoreAttribute");
                if (attrib != null)
                {
                    ignore = true;
                }
                ...
                {
                    attrib = GetAttribute(attribs, "UcAsp.RPC.ProtoBuf.ProtoEnumAttribute");
            ...
            {
                attrib = GetAttribute(attribs, "UcAsp.RPC.ProtoBuf.ProtoMemberAttribute");
                GetIgnore(ref ignore, attrib, attribs, "UcAsp.RPC.ProtoBuf.ProtoIgnoreAttribute");
        ...
        private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
        {
            ...
            if (vm != null)
            {
                ...
                vm.IsMap = ignoreListHandling ? false : vm.ResolveMapTypes(out _, out _, out _);
                if (vm.IsMap) // is it even *allowed* to be a map?
                {
                    if ((attrib = GetAttribute(attribs, "UcAsp.RPC.ProtoBuf.ProtoMapAttribute")) != null)
                    {
                        object tmp;
                ...
                foreach (ValueMember member in fieldsArr)
                {
                    ...
                    else
                    {
                        ...
                        if (member.AsReference)
                        {
                            ...
                            AddOption(builder, ref hasOption).Append("(.UcAsp.RPC.ProtoBuf_net.fieldopt).asRef = true");
                        }
                        if (member.DynamicType)
                        {
                            imports |= RuntimeTypeModel.CommonImports.Protogen;
                            AddOption(builder, ref hasOption).Append("(.UcAsp.RPC.ProtoBuf_net.fieldopt).dynamicType = true");
                        }
