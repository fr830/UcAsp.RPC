...
{
    /// <summary>
    /// Provides UcAsp.RPC.ProtoBuf serialization support for a number of types that can be defined at runtime
    /// </summary>
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        /// <summary>
        /// The default model, used to support UcAsp.RPC.ProtoBuf.Serializer
        /// </summary>
        ...
        public override string GetSchema(Type type, ProtoSyntax syntax)
        {
            ...
            {
                headerBuilder.Append("import \"UcAsp.RPC.ProtoBuf-net/bcl.proto\"; // schema for UcAsp.RPC.ProtoBuf-net's handling of core .NET types");
                Helpers.AppendLine(headerBuilder);
            }
            if ((imports & CommonImports.Protogen) != 0)
            {
                headerBuilder.Append("import \"UcAsp.RPC.ProtoBuf-net/protogen.proto\"; // custom UcAsp.RPC.ProtoBuf-net options");
                Helpers.AppendLine(headerBuilder);
            }
            if ((imports & CommonImports.Timestamp) != 0)
            {
                headerBuilder.Append("import \"google/UcAsp.RPC.ProtoBuf/timestamp.proto\";");
                Helpers.AppendLine(headerBuilder);
            }
            if ((imports & CommonImports.Duration) != 0)
            {
                headerBuilder.Append("import \"google/UcAsp.RPC.ProtoBuf/duration.proto\";");
                Helpers.AppendLine(headerBuilder);
            }
...
//#endif
        ...
        /// (the serialization dll still requires UcAsp.RPC.ProtoBuf-net for support services).
        /// </summary>
        /// <remarks>A full compilation is restricted to accessing public types / members</remarks>
...
,
        ...
        internal void TakeLock(ref int opaqueToken)
        {
            ...
            opaqueToken = Interlocked.CompareExchange(ref contentionCounter, 0, 0); // just fetch current value (starts at 1)
#else
            if (System.Threading.Monitor.TryEnter(types, metadataTimeoutMilliseconds))
            {
                    ...
                    case ProtoTypeCode.DateTime:
                        switch (dataFormat)
                        {
                            ...
                            case DataFormat.WellKnown:
                                ...
                                return ".google.UcAsp.RPC.ProtoBuf.Timestamp";
                            default:
                                imports |= CommonImports.Bcl;
                                return ".bcl.DateTime";
                        }
                    case ProtoTypeCode.TimeSpan:
                        switch(dataFormat)
                        {
                            ...
                            case DataFormat.WellKnown:
                                ...
                                return ".google.UcAsp.RPC.ProtoBuf.Duration";
                            default:
                                imports |= CommonImports.Bcl;
                                return ".bcl.TimeSpan";
                        }
